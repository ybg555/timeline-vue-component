    ...
    >
      <li
        ...
        :class="{
          ...
          'timeline-end': index === (list.length - 1)
        }"
      >
        <span class="item-grouptitle"
          v-if="timeline.groupTile"
          :style="{ width: `${titleWidth}px`, left: `${-spacing}px`}"
        >{{ timeline.groupTile }}</span>

        <span class="item-title"
          :style="{ width: `${titleWidth}px`, left: `${-spacing}px`}"
        >{{ timeline.title }}</span>

        <div class="item-start" v-if="index === 0"
          :style="{
        ...
        ><slot name="startCircle"></slot></div>

        <div :class="[
            'item-circle',
            `item-circle-${index}`,
            `item-circle-${index % 2 === 0 ? 'even' : 'odd'}`,
          ]"
          :style="{
        ...
        <div :class="{ 'item-next': next, 'item-end': true }"
      ...
      <li class="timeline-vertical-bar"
        :style="{ left: `${verticalBarLeft}px`}"
      ></li>
    </ul>
